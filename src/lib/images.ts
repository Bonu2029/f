 ...
 *
 * CURRENT STATE
 *   `src` points at generated on-palette placeholder art in /public/images/art
 *   (see scripts/generate-placeholders.mjs). These are abstract by design — the
 *   site never shows an invented photograph of a person or of client work.
 *
 * TO SWAP IN REAL IMAGERY
 *   1. Generate with the prompts below (Higgsfield `soul_2`, 2k), or use the
 *      client's own photos.
 *   2. Drop the files into public/images/art as <slot-id>.<jpg|webp|avif>.
 *   3. Run `node scripts/adopt-images.mjs` — it rewrites the `src` values here
 *      for every slot it finds a real file for.
 * ========================================================================== */
...
export const IMAGES: Record<string, ImageSlot> = {
  ...
  "hero-primary": slot(
    "hero-primary",
    "A stylist finishing a long, glossy balayage blowout in the salon's main styling room",
    "3:4",
  ...
  "service-color": slot(
    "service-color",
    "Colour application at the salon's colour bar",
    "4:5",
  ...
  "service-balayage": slot(
    "service-balayage",
    "Hand-painted balayage being swept through mid-lengths",
    "4:5",
  ...
  "service-haircut": slot(
    "service-haircut",
    "A precision cut being sectioned at the styling chair",
    "4:5",
  ...
  "service-blowout": slot(
    "service-blowout",
    "A voluminous blowout being finished with a round brush",
    "4:5",
